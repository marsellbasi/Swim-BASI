...
const sourceFiles = (await readdir(sourceDirectory)).filter((file) => /\.jpe?g$/i.test(file));
const unexpectedSources = sourceFiles.filter((file) => !expectedSources.has(file));
...
if (unexpectedSources.length || missingSources.length) {
  const problems = [
    unexpectedSources.length ? `Unexpected sources: ${unexpectedSources.join(', ')}` : '',
...
for (const definition of imageDefinitions) {
  ...
  for (const width of widths) {
    ...
    variants.push({
      ...
      bytes: buffer.length,
      aspectRatio: Number((outputMetadata.width / outputMetadata.height).toFixed(4)),
    });
  }
...
const manifest = {
  ...
  audit: {
    ...
    averageOptimizedBytes: Math.round(optimizedTotalBytes / optimizedOutputCount),
    unexpectedSources,
    missingSources,
  },
  images,
...
const manifestBuffer = Buffer.from(await format(JSON.stringify(manifest), { parser: 'json' }));
