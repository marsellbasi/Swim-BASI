...
for (const match of shopHtml.matchAll(
  ...
  try {
    ...
    errors.push(`shop/index.html has invalid product structured data: ${error.message}`);
  }
}

const productCards = [...shopHtml.matchAll(/<article\b([^>]*class="product-card"[^>]*)>/g)].map(
  (match) => ({
...
if (new Set(productIds).size !== 42) errors.push('Product IDs are missing or duplicated');
if (new Set(productSlugs).size !== 42) errors.push('Product slugs are missing or duplicated');
...
for (const schema of productSchemas) {
  ...
  if (schema.name.endsWith('One-Piece Swimsuit')) {
    ...
    errors.push(`Unrecognized product category in structured data: ${schema.name}`);
  }

  if (schema.url !== `${productBaseUrl}${expectedSlug}`) {
    errors.push(`${schema.name} has a blank, fallback, non-Printful, or slug-conflicting URL`);
  }
  if (schema.offers?.url !== schema.url) {
    errors.push(`${schema.name} structured checkout URL does not match its product URL`);
  }
...
const imageManifest = JSON.parse(
  readFileSync(join(root, 'src', 'data', 'product-image-manifest.json'), 'utf8'),
);
const imageEntries = Object.entries(imageManifest.products);
const galleryPaths = imageEntries.flatMap(([, entry]) => entry.gallery.map((asset) => asset.src));
if (imageEntries.length !== 42 || imageManifest.audit.productMappingCount !== 42) {
  errors.push('Product image manifest does not contain exactly 42 mapped products');
}
...
if (new Set(galleryPaths).size !== galleryPaths.length) {
  errors.push('One or more product gallery assets are mapped to multiple products or views');
}
for (const [slug, entry] of imageEntries) {
  ...
  if (entry.gallery.map((asset) => asset.view).join(',') !== expectedViews.join(',')) {
    errors.push(`${slug} gallery views are not in front, back, left, right order`);
  }
  for (const asset of entry.gallery) {
    if (!existsSync(join(dist, asset.src))) errors.push(`Missing manifest asset: ${asset.src}`);
