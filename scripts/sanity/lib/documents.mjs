...
const block = (text, blockKey = 'body') => [
  {
    ...
    style: 'normal',
    markDefs: [],
    children: [{ _type: 'span', _key: `${key(blockKey)}text`, text, marks: [] }],
  },
];
const link = (destination) =>
  destination.startsWith('http')
    ? { _type: 'link', linkType: 'external', externalUrl: destination, openInNewTab: true }
    : { _type: 'link', linkType: 'internal', internalPath: destination, openInNewTab: false };
...
export function taxonomyDocuments() {
  ...
  const collections = taxonomy.map((item) => ({
    ...
    enabled: true,
    sections: [
      commonSection('richTextSection', `${key(item.slug)}intro`, `${item.title} heading`, {
        eyebrow:
          item.slug === 'one-piece'
            ? 'Starting at $32.99'
      ...
      commonSection('productGridSection', `${key(item.slug)}grid`, `${item.title} products`, {
        heading: item.title,
        source: 'curated',
        products: products
          .filter((product) => product.collection === item.slug)
...
export function productDocuments(assetMap) {
  return products.map((product) => {
    ...
    return {
      ...
      gallery: images.map((image, index) => ({ ...image, _key: `image${index + 1}` })),
      color: { _type: 'productColor', name: product.colorName, hex: product.colorHex },
      categories: [
        {
          _type: 'reference',
  ...
  return [
    {
      ...
      defaultSocialDescription: 'Swimwear designed for women who embody confidence.',
...
export function navigationDocuments() {
  ...
  return [
    { _id: 'headerNavigation', _type: 'headerNavigation', items: navigation.header.map(item) },
    {
...
export function pageDocuments(assetMap) {
  ...
  const brand = (name, alt, options) =>
    managedImage(`public/images/brand/${name}-1400w.webp`, alt, assetMap, options);
    ...
    sections: [
      commonSection('videoSection', 'brandfilm', 'Brand film introduction', {
        ...
        video: {
          ...
          sourceType: 'upload',
          uploadedVideo: assetMap['public/videos/campaigns/swim-basi-brand-film.mp4']?.sanityAssetId
            ? {
                _type: 'file',
                asset: {
                  _type: 'reference',
                  _ref: assetMap['public/videos/campaigns/swim-basi-brand-film.mp4'].sanityAssetId,
                },
              }
            : undefined,
      ...
      commonSection('heroSection', 'mainhero', 'Main campaign hero', {
        ...
        media: { _type: 'responsiveMedia', mediaType: 'image', image: hero, fit: 'cover' },
        primaryCallToAction: cta('Shop Swim BASI', '/shop'),
        secondaryCallToAction: cta('Explore Collections', '/collections', 'secondary'),
        alignment: 'left',
        variant: 'overlay',
      }),
      commonSection('collectionGridSection', 'silhouettes', 'Shop by silhouette', {
      ...
      commonSection('editorialGridSection', 'instagram', 'Follow the color story', {
  ...
  const shop = {
    ...
    sections: [
      ...
      commonSection('productGridSection', 'allproducts', 'Complete product catalog', {
        source: 'curated',
  ...
  const collectionsPage = {
    ...
    sections: [
      commonSection('richTextSection', 'collectionsintro', 'Collections heading', {
        eyebrow: 'Choose your silhouette',
        heading: 'Collections',
        content: block('A confident shape for every way you want to show up.', 'collectionsintro'),
      }),
      commonSection('collectionGridSection', 'collectioncards', 'Collection cards', {
        heading: 'Shop by silhouette',
        collections: collectionRefs,
        variant: 'cards',
      }),
