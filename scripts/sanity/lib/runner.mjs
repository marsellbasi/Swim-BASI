...
const documentMapPath = path.join(
  artifactsDirectory,
  "sanity-document-map.json",
);
const rollbackPath = path.join(
  artifactsDirectory,
  "sanity-rollback-manifest.json",
);
...
const comparableDocument = (value) => {
  ...
  return Object.fromEntries(
    Object.entries(value)
      .filter(
        ([name, child]) =>
          !["_rev", "_createdAt", "_updatedAt"].includes(name) &&
          child !== undefined,
      )
...
const documentsMatch = (left, right) =>
  JSON.stringify(comparableDocument(left)) ===
  JSON.stringify(comparableDocument(right));

async function uploadAssets(mode, inventory) {
  const groupArgument = process.argv
    .slice(2)
  ...
  const inGroup = (item) => {
    ...
    if (group === "brand-film")
      return item.source.includes("/videos/campaigns/");
    if (group === "pages")
      return (
  ...
  const planned = inventory.filter(
    (item) => item.recommendation === "upload" && inGroup(item),
  );
  ...
  for (const item of planned) {
    if (
      map[item.source]?.sha256 === item.sha256 &&
      map[item.source]?.sanityAssetId
    ) {
      reused += 1;
      continue;
    }
    const assetType =
      item.type === "video" || item.type === "captions" ? "file" : "image";
    ...
    const asset = await client.assets.upload(
      ...
      createReadStream(absolutePath),
      {
        ...
        source: {
          id: item.sha256,
          name: "Swim BASI repository migration",
          url: item.source,
        },
      },
    );
...
async function writeDocuments(mode, documents) {
  ...
  for (const source of documents) {
    ...
    documentMap[source._id] = {
      draftId: targetId,
      type: source._type,
      deterministic: true,
    };
    if (!rollback.draftDocumentIds.includes(targetId))
      rollback.draftDocumentIds.push(targetId);
  }
...
export async function runMigration({
  ...
  const assetResult =
    phase === "assets" || phase === "all"
      ...
      : {
          map: await readJson(assetMapPath, {}),
          planned: 0,
          uploaded: 0,
          reused: 0,
        };
  const documents =
    phase === "assets" ? [] : documentsForPhase(phase, assetResult.map);
  ...
  const report = {
    ...
    assets: {
      ...
      codeControlled: inventory.filter(
        (item) => item.recommendation === "keep-code",
      ).length,
      duplicateGroups,
