    ...
    defineField({
      ...
      validation: (rule) =>
        rule.custom((value, context) => {
          if ((context.parent as { linkType?: string })?.linkType !== 'internal') return true;
          if (!value) return 'An internal path is required.';
          return value.startsWith('/') ? true : 'Internal paths must begin with /.';
        }),
    }),
    defineField({
      name: 'externalUrl',
      ...
      validation: (rule) =>
        externalUrl(rule).custom((value, context) =>
          (context.parent as { linkType?: string })?.linkType === 'external' && !value
