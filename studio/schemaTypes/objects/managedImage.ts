...
export const managedImage = defineType({
  ...
  fields: [
    defineField({
      ...
      type: 'string',
      description: 'Helps editors identify this image. It is not shown on the website.',
    ...
    defineField({
      ...
      type: 'string',
      description: 'Describe the purpose of the image without starting with “image of”.',
      hidden: ({ parent }) => parent?.decorative === true,
      validation: (rule) =>
        rule
          .max(180)
          .custom((value, context) =>
            (context.parent as { decorative?: boolean })?.decorative || value?.trim()
              ? true
  ...
  preview: {
    select: { title: 'internalLabel', alt: 'alt', decorative: 'decorative', media: 'image' },
