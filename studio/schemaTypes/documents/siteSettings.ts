...
export const siteSettings = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "string",
      description:
        "Use %s where the page title should appear, for example “%s | Swim BASI”.",
      validation: (rule) =>
        rule
          .required()
          .max(80)
          .custom((value) =>
            value?.includes("%s") ? true : "Include %s for the page title.",
          ),
    }),
    defineField({
  ...
  preview: {
    prepare: () => ({
      title: "Site Settings",
      subtitle: "Global brand and SEO defaults",
    }),
  },
});
