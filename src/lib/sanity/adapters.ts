import type { Product, ProductImageAsset, ProductImageView } from '../../data/products';
...
function imageAsset(
  ...
  return {
    src: urlForSanityImage(image.image).width(1200).fit('max').auto('format').quality(85).url(),
    width,
    height,
    bytes: 0,
    view,
...
export function sanityProductToLocal(product: SanityProduct): Product {
  ...
  const safeCategory =
    category === 'one-piece' || category === 'string-bikinis' || category === 'high-waisted-bikinis'
      ? category
      : 'one-piece';
  ...
  const hover = gallery.find((image) => image.view === 'back') ?? gallery[1] ?? null;
  return {
    id: product._id,
    ...
    collection: product.collections?.[0]?.title || product.categories?.[0]?.title || 'Swimwear',
    ...
    image: primary,
    hoverImage: hover,
    gallery: primary ? [primary, ...gallery.filter((image) => image.src !== primary.src)] : gallery,
    video: null,
