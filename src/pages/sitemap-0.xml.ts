...
const include = (seo?: SeoValue) => seo?.noIndex !== true && seo?.includeInSitemap !== false;

export const GET: APIRoute = async ({ site }) => {
  const origin = site ?? new URL('https://swimbasi.com');
  const [homepage, about, shop, collectionsPage, sizeGuide, collections] = await Promise.all([
  ...
  const urls = routes
    .map(({ path, seo }) => {
      ...
      const priority = seo?.sitemapPriority ? `<priority>${seo.sitemapPriority}</priority>` : '';
